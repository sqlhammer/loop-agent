...
10. GIVEN a database containing an event named `Test Event 1`, WHEN I run `POST /create_event/` with a request body that contains a new event named `Test Event 1`, THEN `Event Manager` returns 500 and an error response that `an event with the name "Test Event 1" already exists`.
...
12. GIVEN a whitelist of match types `kata, combat`, WHEN I run `POST /create_match/` with a request body that contains a new match of type `BJJ`, THEN `Event Manager` returns 500 and an error response `invalid match type`.
