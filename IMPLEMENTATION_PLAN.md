...
- [ ] Task 7: Enforce unique event name in `POST /create_event/` — duplicate name returns HTTP 500 with a body containing `an event with the name "<name>" already exists` (turns AC10 green).
- [ ] Task 8: Implement the Match entity and `POST /create_match/` with the `kata`/`combat` whitelist — valid type returns 200 + match id and type; non-whitelisted type returns HTTP 500 with body `invalid match type` (turns AC11 and AC12 green).
